-- Generated from SQLAlchemy metadata. Bootstrap schema for Alembic revision 0001_initial.
...
CREATE TABLE title_collections (
	title_id UUID NOT NULL, 
...
CREATE TABLE release_audio_tracks (
	release_id UUID NOT NULL, 
	language VARCHAR(32) NOT NULL, 
	label VARCHAR(100), 
	codec VARCHAR(64), 
...
CREATE TABLE release_subtitle_tracks (
	release_id UUID NOT NULL, 
	language VARCHAR(32) NOT NULL, 
	label VARCHAR(100), 
	format VARCHAR(16), 
...
CREATE INDEX ix_collections_parent_id ON collections (parent_id);

CREATE INDEX ix_people_name_en ON people (name_en);

CREATE INDEX ix_plans_active_sort ON plans (active, sort_order);

CREATE INDEX ix_titles_imdb_rating ON titles (imdb_rating);

CREATE INDEX ix_titles_status ON titles (status);

CREATE INDEX ix_titles_release_year ON titles (release_year);

CREATE INDEX ix_titles_kind ON titles (kind);
...
CREATE INDEX ix_storage_files_message ON storage_files (chat_id, message_id);
...
CREATE INDEX ix_notification_jobs_due ON notification_jobs (status, run_at);

CREATE INDEX ix_payment_attempt_order_status ON payment_attempts (order_id, status);

CREATE INDEX ix_wallet_ledger_wallet_created ON wallet_ledger_entries (wallet_id, created_at);

CREATE INDEX ix_payments_order_status ON payments (order_id, status);

CREATE INDEX ix_releases_title_status ON releases (title_id, status);

CREATE INDEX ix_releases_episode_status ON releases (episode_id, status);

CREATE INDEX ix_releases_lookup ON releases (quality, language, subtitle_type, status);

CREATE INDEX ix_watch_history_user_updated ON watch_history (user_id, updated_at);

CREATE INDEX ix_analytics_events_user_created ON analytics_events (user_id, created_at);

CREATE INDEX ix_analytics_events_type_created ON analytics_events (event_type, created_at);

CREATE INDEX ix_download_history_release_created ON download_history (release_id, created_at);

CREATE INDEX ix_download_history_user_created ON download_history (user_id, created_at);

CREATE INDEX ix_transcode_jobs_source_file ON transcode_jobs (source_file_id);

CREATE INDEX ix_transcode_jobs_status ON transcode_jobs (status);
